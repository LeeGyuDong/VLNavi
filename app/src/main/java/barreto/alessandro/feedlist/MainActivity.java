...
public class MainActivity extends AppCompatActivity implements AdapterListFeed.OnClickItemFeed, View.OnClickListener {
    ...
    private void sendPhotoFirebase(String file){
        ...
        StorageReference reference = FirebaseStorage.getInstance().getReference().child(Calendar.getInstance().getTime()+".jpg");
        reference.putFile( uri ).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            @SuppressWarnings("VisibleForTests")
            @Override
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
                DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference().child(FEED_ROOT);   // FEED_ROOT = feed
